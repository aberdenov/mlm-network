<?php
	...
	if (isset($_POST['send'])) {
		...
		$result = db_query("SELECT * FROM users WHERE login = '".$username."' AND password = MD5('".$userpass."') LIMIT 1");
		if (db_num_rows($result) > 0) {
			...
			if (isset($_POST['chk_save'])) {
				...
				if (isset($_COOKIE['korkem_auth'])) {
					$cookie_value = "";
					setcookie("korkem_auth", $cookie_value, 0, "", $_SERVER['HTTP_HOST']);
				}
			}
	...
	if (isset($_COOKIE['korkem_auth'])) {
		$str = crypt_string($_COOKIE['korkem_auth'], false);
